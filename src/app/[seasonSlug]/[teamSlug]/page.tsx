...
export default async function TeamSeasonPage({ params }: TeamSeasonPageProps) {
  ...
  const team = await (async () => {
    ...
    if (!team) {
      console.error(`Team ${teamSlug} not found in season ${seasonSlug}`)
  ...
  try {
    ...
    if (!teamOverallStats) {
      console.error('No team overall stats available', { seasonSlug, teamSlug })
    }
    if (!racesResult) {
      console.error('No races result available', { seasonSlug })
    }
    if (!champion) {
      console.error('No champion data available', { seasonSlug })
    }
    if (!result) {
      console.error('No drivers season stats available', {
        seasonSlug,
        driverOne: team.drivers[0].slug,
        driverTwo: team.drivers[1].slug
      })
    ...
    console.error('Error loading team season page', error)
    return (
      <div className='p-4 text-center'>
        <p>Error loading team season page</p>
        <p className='mt-2 text-sm text-gray-500'>
          Please try again later or contact support if the issue persists.
        </p>
      </div>
